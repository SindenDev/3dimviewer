...
public:
	...
	CReportGenerator(QString fileName);

	/**
	* \brief		Constructs object using already created QPrinter.
	*				! ATTENTION ! :  	Printers setOutputFileName() method must be called before using this constructor, otherwise it will not work!!
	*
	* \param		Page to add.
	*/
	...
	*/
	...
	/**
	* \brief		    Returns pointer to page specified by index
	*
	* \param    index   Index of desired page.
	* \return		    Pointer to page on success, mullptr otherwise.
	*/
	CReportPage* getPage(int index);

    /**
    * \brief		Returns pointer to last page
    *
    * \return		Pointer to last page, mullptr if there is not page.
    */
	...
	*/
	...
	/**
	* \brief				Sets printer used for printing.
	*
	...
	*			
	*
	* \return	Pointer to currently used printer. Can return nullptr.
	*/
	QPrinter* getPrinter();
...
private:
    ...
    std::vector<CReportPage*> m_createdPages;
