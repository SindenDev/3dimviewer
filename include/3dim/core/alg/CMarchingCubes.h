...
private:
    //! Mesh
    data::CMesh m_mesh;
    std::set<std::pair<data::CMesh::VertexHandle, int> > m_openEdgeVertices;
    ...
    data::CMesh::Point m_cube_vertices[8];

    //! Pointer on up work matrices of edges vertices.
    data::CMesh::VertexHandle *m_node_matrix_up_h;
    data::CMesh::VertexHandle *m_node_matrix_up_v;

    //! Pointer on down work matrices of edges vertices.
    data::CMesh::VertexHandle *m_node_matrix_down_h;
    data::CMesh::VertexHandle *m_node_matrix_down_v;

    //! Pointer on middle work matrices of edges vertices.
    data::CMesh::VertexHandle *m_node_matrix_middle;
...
public:
    ...
    data::CMesh &getMesh();
    std::set<std::pair<data::CMesh::VertexHandle, int> > &getOpenEdgeVertices();
    ...
    static void reduceFlatAreas(data::CMesh &mesh, std::vector<data::CMesh::VertexHandle> &vertices, int numOfIterations = 1, bool eliminateNearVertices = true, float maxEdgeLength = 0.0f);
...
protected:
    ...
    void setCubeEdgeNode(int edge_index, int x, int y, data::CMesh::VertexHandle new_vertex);
    ...
    static void makeAllTrisQuality(data::CMesh &mesh);

    //! Markup all vertices of input tri mesh (divide them on three types: flat, edge and corner)
    static void markupVertices(data::CMesh &mesh, std::vector<data::CMesh::VertexHandle> &vertices);
    static void resolveVertexFlag(data::CMesh &mesh, data::CMesh::VertexHandle vertex, OpenMesh::VPropHandleT<int> &vProp_flag, OpenMesh::FPropHandleT<short> &fProp_flag);
    static void resolveNearVertexFlag(data::CMesh &mesh, data::CMesh::VertexHandle vertex, OpenMesh::VPropHandleT<int> &vProp_flag);

    //! Eliminate vertex
    static bool eliminateFlatVertex(data::CMesh &mesh, data::CMesh::VertexHandle eliminate_vertex, float maxEdgeLength);
    static bool eliminateNearVertex(data::CMesh &mesh, data::CMesh::VertexHandle eliminate_vertex, float maxEdgeLength);
    static bool eliminateEdgeVertex(data::CMesh &mesh, data::CMesh::VertexHandle eliminate_vertex, float maxEdgeLength);

    //! Swap all edges of input tri mesh to improve their quality (shape). 
    static void swapTriEdges(data::CMesh &mesh);

    //! Swap maximal edge of input tri to improve his quality.
    static bool swapTriImproveQuality(
        data::CMesh &mesh,
        data::CMesh::FaceHandle swap_tri,
        int loop,
...
public:
    ...
    //! Marching cubes triangle mesh generation.
    bool generateMesh(data::CMesh &mesh, IMarchingCubesFunctor *volumeFunctor, bool reduceFlatAreas, int numOfIterations = 1, bool eliminateNearVertices = true, float maxEdgeLength = 0.0f);

    //! merge sourceMesh into targetMesh
    void mergeMeshes(data::CMesh &sourceMesh, std::set<std::pair<data::CMesh::VertexHandle, int> > &openEdgeVertices, data::CMesh &targetMesh, std::map<int, std::set<std::pair<data::CMesh::VertexHandle, int> > > &vertexMap, std::vector<data::CMesh::VertexHandle> &seamVertices);
};
