...
//    vec3 FinePath = nPath * stepLength(RayPath); 
    ...
    vec3 oColor = log(texel+1.0) / log(MaxVal+1.01); 

	vec4 outputColor = vec4(saturate((oColor * textureSampling * imageAdjustment.y * 25.0) + imageAdjustment.x), 1.0);
    gl_FragData[0] = outputColor;
    float alpha = (outputColor.r + outputColor.g + outputColor.b) / 3.0f;
