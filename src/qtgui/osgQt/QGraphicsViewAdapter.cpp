    ...
    _widget(widget)
{
    ...
    connect(_graphicsScene, SIGNAL(changed(const QList<QRectF> &)), this, SLOT(repaintRequestedSlot(const QList<QRectF> &)));
    connect(_graphicsScene, SIGNAL(sceneRectChanged(const QRectF &)), this, SLOT(repaintRequestedSlot(const QRectF &)));
...
{
    QWidget *childAt = _widget->childAt(pos);
    if (childAt)
    {
        return childAt;
    }
