...
GLWidget::~GLWidget()
{
    ...
    {
        _gw->close();
		if (NULL != dynamic_cast<GraphicsWindowQt*>(_gw.get()))
			dynamic_cast<GraphicsWindowQt*>(_gw.get())->_widget = NULL;
        _gw = NULL;
    }
}
...
GraphicsWindowQt::~GraphicsWindowQt()
{
    ...
    // 3DIM NOTE: this wasn't deleted in the original OSG code, why?
    if (_ownsWidget)
        delete _widget;
}
...
{
    if(height == 0)
        osgViewer::GraphicsWindow::resizedImplementation(x, y, width, 1);
    else
        osgViewer::GraphicsWindow::resizedImplementation(x, y, width, height);
}
