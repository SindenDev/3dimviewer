...
#include <controls/QRoundProgressBar.h>
    ...
    m_updateFlags(UF_PERCENT)
{
	...
	//pal.setBrush(QPalette::AlternateBase, Qt::NoBrush);
	pal.setColor(QPalette::Highlight,QColor(0,160,255));
	pal.setColor(QPalette::Text,QColor(0,160,255));
	pal.setBrush(palette().currentColorGroup(),QPalette::Highlight,QBrush(QColor(0,160,255)));	
	setPalette(pal);	
}
...
void QRoundProgressBar::drawValue(QPainter &p, const QRectF &baseRect, double value, double arcLength)
{
    ...
    {
//		qDebug() << m_nullPosition << " " << m_dataPenWidth << " " << m_outlinePenWidth << " " << arcLength << " " << palette().highlight().color();
...
void QRoundProgressBar::drawText(QPainter &p, const QRectF &innerRect, double innerRadius, double value)
{
    ...
    // !!! to revise
    QFont f(font());
    f.setPixelSize(innerRadius * qMax(0.05, (0.35 - (double)(m_decimals+1) * 0.08)));
    p.setFont(f);
